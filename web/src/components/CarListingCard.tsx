...
function formatCedis(n: number): string {
...
function isLegacyHot(car: CarSubmission): boolean {
  return Boolean(car.car_description?.includes("Highlighted on legacy homepage"));
}

export function CarListingCard({ car }: { car: CarSubmission }) {
  const photos = useMemo(() => normalizePhotos(car.photos), [car.photos]);
  ...
  const waHref = listingEnquiryWaLink({
    id: car.id,
    title,
    price: car.price,
  ...
  return (
    <article className="browse-car-card group flex flex-col overflow-hidden rounded-2xl border border-[#E2E6EA] bg-gradient-to-br from-white via-[#FFFBF8] to-[#F4F9FC] shadow-[0_4px_16px_rgba(232,80,10,0.06)] transition-[transform,box-shadow,border-color] duration-300 hover:-translate-y-1 hover:border-[#E8500A]/35 hover:shadow-[0_14px_32px_rgba(232,80,10,0.12)]">
      <div className="browse-car-media relative aspect-[16/11] w-full overflow-hidden bg-gradient-to-br from-[#FFF0EB] to-[#EEF6FA]">
        {src && !imgErr ? (
          <Image
            src={src}
        ...
        <span className="browse-car-badge absolute right-3 top-3 z-[5] rounded-lg bg-[#00875a] px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-white shadow-md ring-1 ring-white/30 backdrop-blur-sm">
          {badgeLabel(car.status)}
        </span>

        {isLegacyHot(car) ? (
          <span
            className="browse-hot-badge absolute -left-1 -top-1 z-[6] flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-[#FF6B35] to-[#F7931E] text-lg shadow-lg ring-[3px] ring-white"
            title="Featured pick"
            aria-hidden
          >
            🔥
        ...
        {photos.length > 1 ? (
          <>
            ...
            <div
              className="browse-car-dots absolute bottom-3 left-1/2 z-[5] flex -translate-x-1/2 gap-1.5"
              role="group"
              aria-label="Gallery"
            >
              {photos.map((_, i) => (
                <button
                  ...
                  className={`h-2 w-2 rounded-full transition-transform ${i === safeIdx ? "browse-dot-active scale-125" : "bg-white/60 hover:bg-white"}`}
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    goTo(i);
      ...
      <div className="flex flex-1 flex-col bg-gradient-to-b from-white/95 to-[#FAFCFE] p-4">
        <Link href={`/cars/${car.id}`} className="font-display mb-1 line-clamp-2 text-sm font-bold leading-snug text-[#1A1F2E] transition-colors hover:text-[var(--orange)] md:text-[15px]">
          {title}
        </Link>
        ...
        <div className="font-display mb-2 text-lg font-bold text-[var(--orange)] md:text-xl">{formatCedis(car.price)}</div>
        ...
        <div className="mb-3 flex items-start gap-1.5 rounded-lg bg-[#F0F5FF]/90 px-2 py-1.5 text-xs text-[#4B5563] ring-1 ring-[#4a90e2]/15">
          ...
          <span className="line-clamp-2 leading-snug">{car.location}</span>
        </div>
