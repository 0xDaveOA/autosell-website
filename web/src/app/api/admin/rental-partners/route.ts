...
/** Admin-only create — for staff adding a partner directly. */
export async function POST(req: Request) {
  ...
  const input = body as Partial<RentalPartnerInsertInput> & { status?: string; notes?: string };
  const built = buildRentalPartnerRow(input as RentalPartnerInsertInput);
  if (!built.ok) {
    return NextResponse.json({ error: built.error }, { status: 400 });
  }

  if (input.status && isAdminPartnerStatus(input.status)) {
  ...
  const { data, error } = await service.from("rental_partners").insert([built.row]).select("id").maybeSingle();
  if (error) {
    return NextResponse.json({ error: formatSupabaseInsertError(error) }, { status: 422 });
  }

  return NextResponse.json({ ok: true, partnerId: data?.id ?? null });
}
